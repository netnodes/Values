...
function accept() {
  ...
  document.querySelector( '.cookies' ).classList.add('hidden');
  document.cookie = 'cookieConsent=OK; expires=Thu, 31 Dec 2099 23:59:59 UTC; domain=netnodes.net; path=/';
}

function decline() {
  updateCookieDecision('NotOK');
  document.querySelector( '.cookies' ).classList.add('hidden');
  document.cookie = 'cookieConsent=NotOK; expires=Thu, 31 Dec 2099 23:59:59 UTC; domain=netnodes.net; path=/';
  removeCookies();
}
...
function removeCookies() {
  ...
  for (let i = 0; i < cookieNames.length; i++) {
    ...
    if (prefixes.some(prefix => cookieName.indexOf(prefix) === 0)) {
      document.cookie = cookieName + '=delete; expires=Sun, 1 Jan 2023 23:59:59 UTC; domain=netnodes.net; path=/;';
    }
  }
