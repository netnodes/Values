...
function accept() {
  ...
  document.querySelector( '.cookies' ).classList.add('hidden');
  document.cookie = 'cookieConsent=OK; max-age=31536000; domain=netnodes.net; path=/';
}

function decline() {
  updateCookieDecision('NotOK');
  document.querySelector( '.cookies' ).classList.add('hidden');
  document.cookie = 'cookieConsent=NotOK; max-age=31536000; domain=netnodes.net; path=/';
  removeCookies();
}
...
function removeCookies() {
  ...
  for (let i = 0; i < cookieNames.length; i++) {
    ...
    if (prefixes.some(prefix => cookieName.indexOf(prefix) === 0)) {
      document.cookie = cookieName + '=delete; max-age=0; domain=netnodes.net; path=/;';
